      ...
      {tracks.map((track, index) => (
        <motion.div
          ...
          className={`grid grid-cols-[40px,1fr,1fr,minmax(120px,1fr),40px] gap-4 items-center px-4 py-2 rounded group cursor-pointer ${
            currentlyPlaying === track.id ? 'bg-primary bg-opacity-20' : ''
          }`}
        ...
        >
          ...
          <div className="flex items-center justify-center">
            ...
            <motion.button
              ...
              className={`${showIndex ? 'hidden group-hover:flex' : 'flex'} items-center justify-center w-8 h-8 rounded-full bg-primary text-black hover:bg-accent transition-colors`}
            >
              <ApperIcon 
          ...
          <div className="flex items-center space-x-3 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <p className={`font-medium truncate ${
                currentlyPlaying === track.id ? 'text-primary' : 'text-white'
              }`}>
                {track.title}
              </p>
          ...
          <div className="hidden sm:block">
            <p className="text-sm text-gray-400">
              {formatDistanceToNow(new Date(track.createdAt || Date.now()), { addSuffix: true })}
            </p>
          </div>
